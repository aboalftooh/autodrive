...
SUPPORT="$ROOT/scripts/static-v01-support"
OUT="$ROOT/build/v01-static"
mkdir -p "$OUT"
...
# 0) Test inventory gate: every defined test has an explicit execution class.
python3 scripts/test-inventory-v01.py | tee "$OUT/test-inventory.log"
...
BEHAVIOR_SRC=(
  ...
  feature/commission/src/main/kotlin/com/autodrive/app/feature/commission/domain/repository/InvoiceDetailRepository.kt
  feature/commission/src/main/kotlin/com/autodrive/app/feature/commission/domain/usecase/GetInvoiceDetailsUseCase.kt
  ...
  feature/commission/src/test/kotlin/com/autodrive/app/feature/commission/domain/usecase/GetInvoiceDetailsUseCaseTest.kt
...
kotlinc -cp "$COROUTINES_JAR" "${BEHAVIOR_SRC[@]}" \
  -include-runtime -d "$OUT/behavior-v01.jar" 2>&1 | tee "$OUT/behavior-compile.log"
java -cp "$OUT/behavior-v01.jar:$COROUTINES_JAR" BehaviorRunnerKt \
...
kotlinc "${ARCH_SRC[@]}" -include-runtime -d "$OUT/architecture-v01.jar" \
  2>&1 | tee "$OUT/architecture-compile.log"
java -jar "$OUT/architecture-v01.jar" 2>&1 | tee "$OUT/architecture-tests.log"
...
echo "v01 static verification: PASS"

...
class FinalClosureV58ContractTest {
    ...
    @Test
    fun `achievements is the only root performance path and legacy reports root is removed`() {
        val achievementsRoot = root.resolve("feature/achievements/src/main/kotlin/com/autodrive/app/feature/achievements/presentation")
        val viewModel = achievementsRoot.resolve("AchievementsViewModel.kt").readText()
        val screen = achievementsRoot.resolve("AchievementsScreen.kt").readText()
        val destinations = app("navigation/AppDestinations.kt")
        val graphs = app("navigation/NavigationGraphs.kt")

        assertTrue(viewModel.contains("WeeklyPerformanceApi"))
        assertTrue(screen.contains("selectedItemId = \"achievements\""))
        assertTrue(screen.contains("AutoDriveNavigationItem(\"achievements\", \"إنجازاتي\""))
        assertTrue(destinations.contains("data object Achievements"))
        assertTrue(graphs.contains("composable(Screen.Achievements.route)"))

        assertFalse(appRoot.resolve("feature").resolve("reports").exists())
        assertFalse(destinations.contains("ActivityLog"))
        assertFalse(graphs.contains("Screen.ActivityLog"))
    }

...
fun AutoDriveTextField(
    ...
    OutlinedTextField(
        ...
        colors = OutlinedTextFieldDefaults.colors(
            ...
            focusedPlaceholderColor = AutoDriveText.Secondary,
            unfocusedPlaceholderColor = AutoDriveText.Secondary,
        ...
        trailingIcon = {
            when {
                ...
                value.isNotBlank() -> IconButton(onClick = onClear, modifier = Modifier.size(AutoDriveIconSize.TouchTarget)) {
                    Icon(Icons.Rounded.Close, contentDescription = "مسح البحث", modifier = Modifier.size(AutoDriveIconSize.SM))
                }
            }
        },
        shape = AutoDriveRadius.MediumShape,
    ...
    ExposedDropdownMenuBox(
        ...
        ExposedDropdownMenu(expanded = expanded, onDismissRequest = { expanded = false }, containerColor = AutoDriveSurface.Raised) {
            options.forEach { option ->
                DropdownMenuItem(
                    modifier = Modifier.semantics { this.selected = option.id == selected?.id },
                    text = {
                        Text(
                            option.label,

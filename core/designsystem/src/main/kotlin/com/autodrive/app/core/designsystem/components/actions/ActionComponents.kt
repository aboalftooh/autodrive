...
enum class AutoDrivePrimaryButtonTone { Primary, Destructive }
...
fun AutoDrivePrimaryButton(
    ...
    tone: AutoDrivePrimaryButtonTone = AutoDrivePrimaryButtonTone.Primary,
) {
    ...
    val containerColor = when (tone) {
        AutoDrivePrimaryButtonTone.Primary -> AutoDriveBrand.Primary
        AutoDrivePrimaryButtonTone.Destructive -> AutoDriveStatus.Error
    }
    Button(
        onClick = onClick,
        ...
        modifier = modifier
            .height(56.dp)
            .then(if (loading) Modifier.semantics { contentDescription = text; stateDescription = "جارٍ التحميل" } else Modifier)
...
fun AutoDriveTextButton(
    ...
    TextButton(
        ...
        enabled = enabled && !loading,
        modifier = modifier
            .height(AutoDriveIconSize.TouchTarget)
            .then(if (loading) Modifier.semantics { contentDescription = text; stateDescription = "جارٍ التحميل" } else Modifier),
        shape = AutoDriveRadius.MediumShape,
    ...
    IconButton(
        ...
        modifier = modifier
            ...
            .semantics {
                ...
                if (loading) stateDescription = "جارٍ التحميل"
            },
        ...
        if (loading) CircularProgressIndicator(modifier = Modifier.size(AutoDriveIconSize.SM), strokeWidth = AutoDriveBorder.Strong, color = color)
        else Icon(icon, contentDescription = null, modifier = Modifier.size(AutoDriveIconSize.MD))
    }
}
...
fun AutoDriveFab(
    ...
    Button(
        ...
        else Icon(icon, contentDescription = null, modifier = Modifier.size(AutoDriveIconSize.LG))
    }
}

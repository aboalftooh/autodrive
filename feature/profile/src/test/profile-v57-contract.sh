...
require(all(x in screen for x in ('"home" -> onNavigateHome()', '"messages" -> onNavigateRecent()', '"achievements" -> onNavigateAchievements()', 'selectedItemId = "settings"')), 'bottom navigation behavior is preserved')
